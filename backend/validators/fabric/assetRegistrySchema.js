...
const assetIdParam = joi
  .object({
    id: joi.string().trim().min(1).max(128).required(),
  })
  .required();

const createAssetSchema = joi
  .object({
    body: joi
      .object({
        id: joi.string().trim().min(1).max(128).required(), // <-- Move id here
...
const assetReadSchema = joi
  .object({
    params: assetIdParam,
  })
  .required();

const assetDeleteSchema = joi
  .object({
    params: assetIdParam,
  })
  .required();
