...
async function createChannel(workspace, userId, config, opts = {}) {
  ...
  const channelTxPath = path.join(
    getChannelArtifactsDir(workspace),
    `${channelId}.tx`,
  );
  ...
  logger.info(
    `[INFO] Creating channel ${channelId} for ${getProjectNetwork(userId)}...`,
  );
...
async function joinPeersToChannel(workspace, userId, config, opts = {}) {
  ...
  logger.info(
    `[INFO] Joining peers to ${channelId} for ${getProjectNetwork(userId)}...`,
  );
  ...
  for (const org of config.orgs || []) {
    for (let peerIndex = 0; peerIndex < (org.peerCount || 0); peerIndex++) {
      ...
      logger.debug("[DEBUG] channel join peer", {
        org: org.name,
        peerIndex,
        peerAddress,
        channelId,
...
async function updateAnchorPeers(workspace, userId, config, opts = {}) {
  ...
  logger.info(
    `[INFO] Updating anchor peers for ${channelId} in ${getProjectNetwork(userId)}...`,
  );
  if (progress) await progress({ step: "channel.anchor.start", channelId });

  for (const org of config.orgs || []) {
    const anchorTxPath = await generateAnchorPeerUpdateTx(
      workspace,
      config,
      org,
    );
    await assertFileExists(
      anchorTxPath,
      "Anchor peer update transaction not found",
    );
    ...
    logger.debug("[DEBUG] channel anchor update", {
      org: org.name,
      channelId,
      anchorTxPath,
    });
    ...
    if (progress)
      await progress({ step: "channel.anchor.org", channelId, org: org.name });
  }
