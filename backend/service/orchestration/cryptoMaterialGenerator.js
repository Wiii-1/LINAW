...
async function generateCryptoMaterial(workspace, userId, config, opts = {}) {
    ...
    */
    ...
    await assertFileExists(path.join(workspace, tlsCaCertRel), 'TLS-CA server cert not found (is composeUpCA running?)');
    await assertFileExists(path.join(workspace, ordererCaCertRel), 'Orderer CA server cert not found (is composeUpCA running?)');

    for (const org of (config.orgs || [])) {
        await assertFileExists(path.join(workspace, `crypto-config/${org.name}/ca/ca-cert.pem`), `Org CA server cert not found for ${org.name}`);
    }
    ...
    for (const org of (config.orgs || [])) {
        for (let i = 0; i < org.peerCount; i++) {
            ...
            try {
                await registerIdentity(workspace, userId, `${clientRoot}/tls-ca-admin`, tlsCaUrl, tlsCaCertRel, idName, idSecret, 'peer');
            } catch (err) {
    ...
    for (let i = 0; i < ordererCount; i++) {
        ...
        try {
            await registerIdentity(workspace, userId, `${clientRoot}/tls-ca-admin`, tlsCaUrl, tlsCaCertRel, idName, idSecret, 'orderer');
        } catch (err) {
            if (!/already registered|exists/i.test(err.message)) throw err;
        }
    ...
    try {
        await registerIdentity(workspace, userId, `${clientRoot}/orderer-ca-admin`, ordererCaUrl, ordererCaCertRel, ordererOrgAdminId, ordererOrgAdminSecret, 'admin');
    } catch (err) {
        if (!/already registered|exists/i.test(err.message)) throw err;
    }
    ...
    for (let i = 0; i < ordererCount; i++) {
        ...
        const idSecret = secretFor(idName);
        try {
            await registerIdentity(workspace, userId, `${clientRoot}/orderer-ca-admin`, ordererCaUrl, ordererCaCertRel, idName, idSecret, 'orderer');
        } catch (err) {
            if (!/already registered|exists/i.test(err.message)) throw err;
        }
    ...
    for (const org of (config.orgs || [])) {
        ...
        try {
            await registerIdentity(workspace, userId, `${clientRoot}/org-ca-admin-${org.name}`, orgCaUrl, orgCaCertRel, orgAdminId, orgAdminSecret, 'admin');
        } catch (err) {
            if (!/already registered|exists/i.test(err.message)) throw err;
        }
        ...
        for (let i = 0; i < org.peerCount; i++) {
            ...
            try {
                await registerIdentity(workspace, userId, `${clientRoot}/org-ca-admin-${org.name}`, orgCaUrl, orgCaCertRel, peerId, peerSecret, 'peer');
            } catch (err) {
                if (!/already registered|exists/i.test(err.message)) throw err;
            }
