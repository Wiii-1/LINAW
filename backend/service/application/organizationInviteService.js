...
class organizationInviteService {
  async createInvite({
    ...
    const invitedMembership =
      await organizationUserDao.findByOrganizationAndUser({
        organization_id,
    ...
    const activeInvite =
      await organizationInviteDao.findActiveByOrganizationAndEmail({
        organization_id,
  ...
  async acceptInvite({ token, user_id, tenant_id }) {
    ...
    if (invite.tenant_id !== tenant_id) {
      throw new AppError(
        "Invitationn does not belong to this tenant",
        403,
        "INVALID_TENANT_INVITATION",
      );
    }
