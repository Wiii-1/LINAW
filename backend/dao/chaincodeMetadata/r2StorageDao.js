...
class R2StorageDao {
  async upload({ key, buffer, contentType, metadata = {} }) {
    try {
      if (!key) {
        throw new Error(
          "Key is required for uploading an object to R2 storage.",
        );
      }

      if (!buffer) {
        throw new Error(
          "Buffer is required for uploading an object to R2 storage.",
        );
      }
      ...
      console.error(
        "[R2ObjectStorageDao] Error uploading object to R2 storage:",
        {
          key,
          contentType,
          error: error.message,
  ...
  async delete({ objectKey }) {
    try {
      if (!objectKey) {
        throw new Error(
          "Object key is required for deleting an object from R2 storage.",
        );
      }
      ...
      console.error(
        "[R2ObjectStorageDao] Error deleting object from R2 storage:",
        {
          objectKey,
          error: error.message,
        },
