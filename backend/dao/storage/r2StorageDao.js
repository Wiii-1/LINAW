...
class R2StorageDao {
  async upload({ bucketName, key, buffer, contentType, metadata = {} }) {
    try {
      if (!bucketName) {
        throw new Error('Bucket name is required for uploading an object to R2 storage.');
      }
  ...
  async delete({ bucketName, objectKey }) {
    try {
      if (!bucketName) {
        throw new Error('Bucket name is required for deleting an object from R2 storage.');
      }
