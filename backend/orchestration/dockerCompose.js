...
async function composeStop(workspace, userId) {
    await assertComposeFileExists(workspace)
    logger.info(`[INFO] Stopping services for ${getProjectName(userId)} (resumable)...`)
    await execAsync(`${composeCmd(workspace, userId)} stop`)
}

async function composeStart(workspace, userId) {
    await assertComposeFileExists(workspace)
    logger.info(`[INFO] Starting services for ${getProjectName(userId)} (resuming)...`)
    await execAsync(`${composeCmd(workspace, userId)} start`)
}

async function listServices(workspace, userId) {
    await assertComposeFileExists(workspace)
    const { stdout } = await execAsync(`${composeCmd(workspace, userId)} config --services`)
    return stdout
        .split('\n')
        .map(s => s.trim())
        .filter(Boolean)
...
async function composeUpCA(workspace, userId) {
    logger.info(`[INFO] Starting CA container(s) for ${getProjectName(userId)}...`)
    const services = await listServices(workspace, userId)
    const caServices = services.filter(s => s.startsWith('tls-ca-') || s.startsWith('ca-'))
    if (!caServices.length) {
        throw new Error(`No CA services found for ${getProjectName(userId)} (compose config --services returned none matching ca-)`)
    }
...
async function composeDownWithVolumes(workspace, userId) {
    await assertComposeFileExists(workspace)
    logger.info(`[INFO] Destroying container + volumes for ${getProjectName(userId)}...`)
...
async function getContainers(workspace, userId) {
    ...
    try {
        const { stdout } = await execAsync(`${composeCmd(workspace, userId)} ps --format json`)
        ...
        logger.debug('[DEBUG] compose ps --format json not available, falling back', { message: err.message })
    }

    const ids = await getContainerIds(workspace, userId)
    const containers = []
    for (const id of ids) {
        const { stdout: nameStdout } = await execAsync(`docker inspect --format '{{.Name}}' ${id}`)
