...
async function generateGenesisBlock(workspace, config = {}) {
	...
	logger.debug('[DEBUG] configtxgen: generating genesis.block', { workspace, systemChannelId });
...
async function generateChannelTx(workspace, config = {}) {
	...
	logger.debug('[DEBUG] configtxgen: generating channel tx', { workspace, channelId });
	await runConfigtxgen(
		workspace,
		[
			'set -euo pipefail',
...
async function generateAnchorPeerUpdateTx(workspace, config = {}, org) {
	...
	if (!org || !org.mspId || !org.name) {
		throw new Error('generateAnchorPeerUpdateTx requires org with { name, mspId }');
	}

	const outFile = `${org.name}-anchors.tx`;
	logger.debug('[DEBUG] configtxgen: generating anchor peer update', { workspace, channelId, org: org.name });
...
module.exports = { generateGenesisBlock, generateChannelTx, generateAnchorPeerUpdateTx }